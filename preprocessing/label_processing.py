...
def create_labels(folderName: str = "data") -> pd.DataFrame:
    ...
    labels_df = pd.DataFrame(labels.items(), columns=["match_turn", "player1wins"])
    labels_df[["match", "turnNumber", "player1wins"]] = pd.DataFrame(
        labels_df.player1wins.tolist()
    )
    labels_df["player1wins"] = labels_df["player1wins"].replace(
        {"Bot 1": 1, "Bot 2": 0, "": pd.NA}
    )
    labels_df = labels_df[labels_df["player1wins"].notna()]
    return labels_df
...
def _createTurnLabels(dataPath: str, matchMap: Dict[str, List[int]]) -> Dict[str, List]:
    ...
    for match, turnList in matchMap.items():
        ...
        for turnNumber in turnList:
            if turnNumber == last:  # The final turn should not be in the data
                continue
            matchTurn = f"{match}-turn{turnNumber}.txt"
            labels[matchTurn] = [match, turnNumber, outcome]
    return labels